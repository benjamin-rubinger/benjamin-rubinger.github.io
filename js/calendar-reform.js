...
function formatJulianDays(d) {
    ...
    const $julianDays = $('div.julian-days');
    $julianDays.text(jdn.toString());
}

function formatJulianCalendar(d) {
    const $julianCalendar = $('div.julian-calendar-date');
...
function formatOrdinalDate(d) {
    const $ordinalDate = $('div.ordinal-date');
    const yearNumber = getYearNumber(d);
...
function updateDatetimeEvent(e) {
    ...
    console.log('update date time event');
    console.log($target);
...
// todo use table of leap seconds
...
//    console.log('get proposal string fixed 5');
    ...
    const ordinalTime = (ordinal + fraction).toFixed(5);
    return `${year} ${ordinalTime}`;
}
...
// from https://www.nist.gov/pml/time-and-frequency-division/time-realization/leap-seconds
...
function initializeCalendarReform() {
